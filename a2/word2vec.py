...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    y_cap = softmax(outsideVectors @ centerWordVec)
    loss = -np.log(y_cap[outsideWordIdx])
    u_o = outsideVectors[outsideWordIdx]
    gradCenterVec = -u_o + (outsideVectors.T @ y_cap)
    y = np.zeros(outsideVectors.shape[0])
    y[outsideWordIdx] = 1.0
    gradOutsideVecs = np.tile(centerWordVec, (outsideVectors.shape[0],1)) * (y_cap - y).reshape(y_cap.shape[0], 1)
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
   ...
   # Negative sampling of words is done for you. Do not modify this if you
    ...
    u_o = outsideVectors[outsideWordIdx]
    loss = -np.log(sigmoid(u_o.T @ centerWordVec)) -np.sum(np.log(sigmoid(-outsideVectors[negSampleWordIndices] @ centerWordVec)), axis=0)
    gradCenterVec =  -u_o * (1.0 - sigmoid(u_o.T @ centerWordVec)) + outsideVectors[negSampleWordIndices].T @ (1.0 - sigmoid(-outsideVectors[negSampleWordIndices] @ centerWordVec))
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = -centerWordVec * (1 - sigmoid(u_o.T @ centerWordVec))
    unique_k_indices, sample_counts = np.unique(negSampleWordIndices, return_counts=True)
    interim_result = (np.tile(centerWordVec, (unique_k_indices.shape[0],1)) * sample_counts[: , np.newaxis] ) * (1.0 - sigmoid(-outsideVectors[unique_k_indices] @ centerWordVec))[:, np.newaxis]
    gradOutsideVecs[unique_k_indices] = interim_result
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        outsideWordIdx = word2Ind.get(outsideWord)
        ...
        loss += currLoss
        gradCenterVecs[word2Ind.get(currentCenterWord)] += currGradCenterVecs
