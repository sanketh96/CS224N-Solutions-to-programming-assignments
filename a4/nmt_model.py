...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        packed_padded_sequence_input = nn.utils.rnn.pack_padded_sequence(X, source_lengths)
        packed_padded_sequence_output, (last_hidden, last_cell) = self.encoder(packed_padded_sequence_input)
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(packed_padded_sequence_output)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = torch.bmm(enc_hiddens.permute(0,2,1), alpha_t.unsqueeze(-1)).squeeze(-1)
