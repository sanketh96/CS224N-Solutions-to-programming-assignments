...
def pad_sents(sents, pad_token):
    ...
    max_pad_length = max([len(sent) for sent in sents])
    sents_padded = [sent + (max_pad_length - len(sent))*[pad_token] for sent in sents]
    min_length = min([len(sent) for sent in sents_padded])
    assert max_pad_length == min_length
    ### END YOUR CODE
