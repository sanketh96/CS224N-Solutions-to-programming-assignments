...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        B, T, C = x.size()
        x_a = self.w1(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        x_a = torch.nn.functional.relu(x_a)
        w2 = self.w2[:, :T] # we want to truncate to T timesteps, not keep full block size
        b2 = self.b2[:T]
        att = x_a @ w2 + b2 # (B, nh, T, hs) @ (hs, T) -> (B, nh, T, T)
        att = att.masked_fill(self.mask[:,:,:T,:T] == 0, -1e10) # todo: just use float('-inf') instead?
        att = F.softmax(att, dim=-1)
        att = self.attn_drop(att)
        v = self.value(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
