...
if args.variant == 'vanilla':
    ...
    mconf = model.GPTConfig(pretrain_dataset.vocab_size, pretrain_dataset.block_size,n_layer=4, n_head=8, n_embd=256, synthesizer=True)
...
if args.function == 'pretrain':
    ...
    if args.reading_params_path is not None:
        # TODO [part c]: Load the model parameters from args.reading_params_path
        # and set the model to eval mode
        model.load_state_dict(torch.load(args.reading_params_path))
    trainer = trainer.Trainer(model=model, train_dataset=train_dataset, test_dataset=None,config=tconf)
    trainer.train()
    torch.save(model.state_dict(), args.writing_params_path)
    # trainer = trainer.Trainer(vanilla_model, tconf, device)
