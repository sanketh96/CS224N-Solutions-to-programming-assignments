...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        document = document[:random_truncation_length]
        masked_length = abs(round(random.normalvariate(round(len(document)/4), 0.5)))
        # masked_length = masked_length if masked_length == 0 else 1
        prefix_length = (len(document) - masked_length)//2
        ...
        try:
          ...
          print("Random truncation length : ", random_truncation_length)
          print("Normal dist value : ", round(random.normalvariate(round(len(document)/4), 0.5)))
          print("Masked length : ", masked_length)
          print("Prefix : ", prefix, " length : ", len(prefix))
          print("Masked content : ", masked_content, " length : ", len(masked_content))
          print("Suffix : ", suffix, " length : ", len(suffix))
          raise AssertionError("Look at print statements")
